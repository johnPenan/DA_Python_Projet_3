...
"""
    def move(self, direction):
        ...
        if (
            self.labyrinth.is_streets(new_position)
            and new_position not in self.labyrinth.object_positions
        ):
            if new_position != self.labyrinth.arrival:
                self.position = new_position
            elif new_position == self.labyrinth.arrival \
                and len(self.items) == 3:
